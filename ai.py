...
class GameState(Enum):
    ...
    BONUS = "BONUS"           # 追击特殊豆子模式

class PacmanAI:
    def __init__(self):
        self.board_size = None
        self.current_state = GameState.COLLECT
        self.path = []
        ...
        # 状态权重
        self.weights = {
            GameState.COLLECT: {"ghost": 1.0, "bean": 1.0, "bonus": 1.5},
            GameState.ESCAPE: {"ghost": 2.0, "bean": 0.5, "bonus": 1.0},
            GameState.BONUS: {"ghost": 0.8, "bean": 0.3, "bonus": 2.0}
        }
    ...
    def update_state(self, game_state):
        ...
        if min_ghost_distance < self.GHOST_DANGER_DISTANCE and not has_shield:
            self.current_state = GameState.ESCAPE
        elif special_bean and special_bean[1] < 8:
            self.current_state = GameState.BONUS
        else:
            self.current_state = GameState.COLLECT
    ...
    def evaluate_position(self, pos, game_state):
        ...
        # 如果是逃离模式，确保远离幽灵
        if self.current_state == GameState.ESCAPE:
    ...
    def choose_move(self, game_state) -> List[int]:
        """选择移动方向"""
